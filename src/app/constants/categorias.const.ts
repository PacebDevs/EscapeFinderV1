export interface Categoria {
    nombre: string;
    icono: string;
  }
  
  export const CATEGORIAS: Categoria[] = [
    { nombre: 'Todo', icono: 'assets/categorias/varita-magica.png' },
    { nombre: 'Fantasía', icono: 'assets/categorias/book.png' },
    { nombre: 'Aventuras', icono: 'assets/categorias/map.png' },
    { nombre: 'Terror', icono: 'assets/categorias/skull.png' },
    { nombre: 'Ciencia Ficción', icono: 'assets/categorias/ufo.png' },
    { nombre: 'Historia', icono: 'assets/categorias/greek-pillars.png' },
    { nombre: 'Apocalíptico', icono: 'assets/categorias/nuclear-bomb.png' },
    { nombre: 'Misterio', icono: 'assets/categorias/footprint.png' },
    { nombre: 'Policíaca', icono: 'assets/categorias/police-car.png' },
    { nombre: 'Criminal', icono: 'assets/categorias/money-bag_02.png' },
    { nombre: 'Adultos', icono: 'assets/categorias/wine.png' },
    { nombre: 'Niños', icono: 'assets/categorias/cubes.png' },
  ];
  